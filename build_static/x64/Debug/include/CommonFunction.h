...
#define HANDLE_DECODE_ERROR(ret, picture, CurrMbAddr) \
    do { \
        if (ret != 0) { \
            if (CurrMbAddr > 0) { \
                /* 将上一个宏块的值复制到当前宏块 */ \
                picture.m_mbs[CurrMbAddr] = picture.m_mbs[CurrMbAddr - 1]; \
                printf("%s(%d): %s: Error: ret=%d; Copying previous macroblock data.\n", \
                       __FILE__, __LINE__, __FUNCTION__, ret); \
            } else { \
                /* 如果是第一个宏块，无法复制上一个宏块的数据 */ \
                printf("%s(%d): %s: Error: ret=%d; Cannot copy previous macroblock data (first macroblock).\n", \
                       __FILE__, __LINE__, __FUNCTION__, ret); \
                return ret; /* 返回错误码 */ \
            } \
            continue; /* 跳过当前宏块的剩余逻辑，继续处理下一个宏块 */ \
        } \
    } while (0)
