...
int CH264PictureBase::convertYuv420pToBgr24_ffmpeg(uint32_t width, uint32_t height, const uint8_t* yuv420p, uint8_t* bgr24, uint32_t widthBytesBgr24)
{
    int ret = 0;
    ...
    // 检查 W 和 H 是否为偶数
    if (W % 2 != 0 || H % 2 != 0) {
        printf("Error: Width and height must be even for YUV420P format\n");
        return -1;
    }

    // 假设 yuv420p 是紧密排列的 YUV420P 数据
    const uint8_t* yData = yuv420p;                     // Y 分量
    const uint8_t* uData = yuv420p + W * H;             // U 分量
    const uint8_t* vData = yuv420p + W * H + (W / 2) * (H / 2); // V 分量
    ...
    for (int y = 0; y < H; ++y)
    {
        ...
        {
            // 计算 YUV 分量的索引
            unsigned char Y = yData[y * W + x];
            unsigned char U = uData[(y / 2) * (W / 2) + x / 2];
            unsigned char V = vData[(y / 2) * (W / 2) + x / 2];
            ...
            // 将 BGR 值写入输出缓冲区
            bgr24[y * widthBytesBgr24 + x * channels + 0] = CLIP3(0, 255, b); // B
            bgr24[y * widthBytesBgr24 + x * channels + 1] = CLIP3(0, 255, g); // G
            bgr24[y * widthBytesBgr24 + x * channels + 2] = CLIP3(0, 255, r); // R
        }
    }
...
int CH264PictureBase::saveToBmpFile(const char *filename)
{
    ...
    ret = convertYuv420pToBgr24(W, H, m_pic_buff_luma, (uint8_t*)bitmap.bmBits, bitmap.bmWidthBytes);//使用内置解码器
    //ret = convertYuv420pToBgr24_ffmpeg(W, H, m_pic_buff_luma, (uint8_t*)bitmap.bmBits, bitmap.bmWidthBytes);//使用ffmpeg解码器
    if (ret != 0)
    {
        return -1;
    }
